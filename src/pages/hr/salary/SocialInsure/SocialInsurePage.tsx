...
function SocialInsurePage() {
  ...
  // const [dataTest, settest] = useState([]); //리덕스 사용
    ...
    <Page title="사회보험관리">
      <Grid container spacing={gridSpacing}>
        <Grid item xs={12}>
          ...
          &nbsp;&nbsp;&nbsp;
          <MyButton variant="contained" color="#5F00FF" onClick={searchInsure(searchYear)} className="button" inputText="조회하기" />
        </Grid>
        <Grid item xs={12}>
          <MyAppBar position="static" value={value} onChange={insureChange} label={labelList} />
          {/* {value === 0 && <MySocialInsure insureName={labelList[0]} dataTest={dataTest} columns={healthColums} values={healthValues} />}
          {value === 1 && <MySocialInsure insureName={labelList[1]} dataTest={dataTest} columns={nationColums} values={nationValues} />}
          {value === 2 && <MySocialInsure insureName={labelList[2]} dataTest={dataTest} columns={industColumns} values={industValues} />}
          {value === 3 && <MySocialInsure insureName={labelList[3]} dataTest={dataTest} columns={empColumns} values={empValues} />} */}
